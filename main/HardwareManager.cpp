...
void HardwareManager::begin()
{
  ...
  // --- Initialize WS2813 ---
  if (m_miscConfig.ws2813Pin != 255)
  {
    size_t ws28143pixelTypeIndex = 5; //"GRB"
    // HardwareManager.cpp
  ...
  // WS2813 Timer Call
  static TimerContext pixelwsF_context = {this, TimerSources::PIXEL_WS_F};
  const esp_timer_create_args_t pixelwsF_timer_args = {
      .callback = &handleTimer,
      .arg = (void *)&pixelwsF_context,
      .name = "wspixelFailTimer"};
  static TimerContext pixelwsS_context = {this, TimerSources::PIXEL_WS_S};
  const esp_timer_create_args_t pixelwsS_timer_args = {
      .callback = &handleTimer,
      .arg = (void *)&pixelwsS_context,
      .name = "wspixelSuccessTimer"};
  ...
  // WS2813 task timer
  esp_timer_create(&pixelwsF_timer_args, &ws_pixelFailTimer);
  esp_timer_create(&pixelwsS_timer_args, &ws_pixelSuccessTimer);
...
void HardwareManager::handleTimer(void *arg)
{
  ...
  case TimerSources::ALT_GPIO_INIT:
    ...
    break;
    // WS2813
  case TimerSources::PIXEL_WS_S:
  case TimerSources::PIXEL_WS_F:
    ESP_LOGD(TAG, "PAUSE Ambient mode");
  ...
  while (true)
  {
    if (xQueueReceive(m_feedbackQueue, &feedback, portMAX_DELAY))
    {
      ...
      case FeedbackType::SUCCESS:
        ...
        if (esp_timer_is_active(m_gpioSuccessTimer))
          esp_timer_stop(m_gpioSuccessTimer);
        if (esp_timer_is_active(m_pixelSuccessTimer))
          esp_timer_stop(m_gpioFailTimer);
        if (m_pixel != nullptr)
        {
      ...
      case FeedbackType::FAILURE:
        ...
        if (esp_timer_is_active(m_gpioFailTimer))
          esp_timer_stop(m_gpioFailTimer);
        if (esp_timer_is_active(m_pixelFailTimer))
          esp_timer_stop(m_pixelFailTimer);
        if (m_pixel != nullptr)
        {
...
WsLedPixel *HardwareManager::getWsLed()
{
  return ws_pixel;
}